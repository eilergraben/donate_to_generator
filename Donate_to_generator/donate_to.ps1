...
for ($i = 0; $i -lt $content.Count; $i++) {
    ...
    if ($line -like 'Donor:*') {
        ...
        if (-not $signature) {
            Write-Host ('WARNUNG: Keine Signiture-Zeile fuer Donor {0} gefunden - Eintrag wird uebersprungen.' -f $donor) -ForegroundColor Yellow
...
if ($donorEntries.Count -eq 0) {
    Write-Host 'Keine Donor/Signiture-Eintraege gefunden.' -ForegroundColor Yellow
